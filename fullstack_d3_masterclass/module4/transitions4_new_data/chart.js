...
async function drawLineChart() {
  ...
  const drawLine = (dataset) => {
    ...
    const lastTwoDataPoints = dataset.slice(-2)

    const pixelsBetweenLastTwoDataPoints = 
        xScale(xAccessor(lastTwoDataPoints[1])) - xScale(xAccessor(lastTwoDataPoints[0]))   
    ...
    const line = bounds.select(".line")
        ...
        .style("transform", `translateX(
            ${pixelsBetweenLastTwoDataPoints}px
        )`)
        .transition()
